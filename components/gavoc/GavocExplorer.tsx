...
import { GavocTable } from './GavocTable';
import { GavocThesaurusTable } from './GavocThesaurusTable';

// eslint-disable-next-line @typescript-eslint/naming-convention
const GavocMap = dynamic(() => import('./GavocMap'), {
  ssr: false,
  loading: () => (
...
export function GavocExplorer(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (gavocData.thesaurus) {
      let thesaurusEntries: GavocThesaurusEntry[] = gavocData.thesaurus.entries;
  ...
  const handleThesaurusEntrySelect = useCallback(
    (entryId: string | null) => {
      ...
      if (entryId && gavocData?.thesaurus) {
        const entry = gavocData.thesaurus.entries.find((e) => e.id === entryId);
        if (entry && entry.locations.length > 0) {
          let selectedLocation: GavocLocation | undefined = entry.locations[0];
