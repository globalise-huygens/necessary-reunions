...
function AnnotationEditor({
  ...
  setPendingGeotags: React.Dispatch<React.SetStateAction<Record<string, any>>>;
  toast: any;
}) {
  ...
  useEffect(() => {
    if (onAnnotationInLinkingMode) {
      if (activeTab === 'link') {
        ...
        // Clear selectedIds when leaving Link tab
        if (setSelectedIds) {
          setSelectedIds([]);
        }
      }
    }
  }, [activeTab, annotation.id, onAnnotationInLinkingMode, annotations]);
  ...
  const handleSaveLinkingData = async () => {
    if (!session) {
    ...
    try {
      ...
      toast({
        title: 'Link Saved!',
        description: 'Annotation link has been successfully saved.',
      });
    } catch (error: any) {
  ...
  const handleSaveGeotagData = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Geographic location has been successfully saved.',
      });
    } catch (error: any) {
  ...
  const handleSavePointData = async () => {
    ...
    try {
      ...
      toast({
        title: 'Point Saved!',
        description: 'Point selector has been successfully saved.',
      });
    } catch (error: any) {
      setSaveError(error.message || 'Failed to save point');
      toast({
        title: 'Save Failed',
        description: error.message || 'Failed to save point. Please try again.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  const handleRemoveData = async (type: 'link' | 'geotag' | 'point') => {
    ...
    try {
      ...
      if (type === 'link') {
        ...
        if (!hasOtherData) {
          const response = await fetch(`/api/annotations/${existingLink.id}`, {
            method: 'DELETE',
            headers: {
              'If-Match': getEtag(existingLink.id) || '',
            },
        ...
        const hasLinkedTargets =
          Array.isArray(existingLink.target) && existingLink.target.length > 1;
    ...
    <div className="space-y-4">
      <div className="p-3 bg-muted/20 border border-border rounded-lg">
        ...
        <div className="grid grid-cols-1 gap-2 text-xs">
          <div className="flex justify-between">
            ...
            <span className="font-mono">{annotation.id.split('/').pop()}</span>
          </div>
          <div className="flex justify-between">
              ...
              {(() => {
                const currentCount =
                  activeTab === 'link' && selectedIds && selectedIds.length > 0
                    ? selectedIds.length
                    : linkedIds.length;
                return currentCount > 0 ? `${currentCount} connected` : 'none';
              })()}
            </span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between p-3 bg-muted/10 border border-border rounded-lg">
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() => {
              if (activeTab === 'link') handleSaveLinkingData();
              else if (activeTab === 'geotag') handleSaveGeotagData();
              else if (activeTab === 'point') handleSavePointData();
            }}
          ...
          >
            {isSaving ? (
              ...
              (() => {
                ...
                if (activeTab === 'geotag')
                  return hasExistingData ? 'Update Location' : 'Save Location';
                if (activeTab === 'point')
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-blue-700 mt-1">
                    ...
                    reading sequence, or ✕ button to remove annotations from the
                    selection.
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 p-3 bg-secondary/10 border border-secondary/30 rounded-lg">
              ...
              {selectedIds && selectedIds.length > 0 ? (
                ...
                <div className="p-3 text-center text-muted-foreground bg-card/50 rounded border border-dashed border-muted-foreground/30">
                  <p className="text-sm">No annotations selected for linking</p>
              ...
              <div className="text-xs text-primary bg-secondary/10 p-2 rounded border border-secondary/30">
                💡 Click annotations in the image viewer to add/remove them from
                this link. Use ↑↓ arrows to arrange annotations in reading
                order, or ✕ button to remove them from the selection.
              </div>
            </div>
          </div>
        )}
...
export function AnnotationList({
  ...
  // Helper functions for annotation capabilities
  const getAnnotationCapabilities = (annotation: any) => {
    const geotagAnno = getGeotagAnnoFor(annotation.id);
    const linkedIds = getLinkedAnnotationIds(annotation.id);

    // Check if has point selector
    const hasPointSelector = (() => {
      ...
      const relatedLinkingAnnotations = allLinkingAnnotations.filter(
        (linkAnno) => {
          if (Array.isArray(linkAnno.target)) {
            return linkAnno.target.includes(annotation.id);
          } else if (linkAnno.target === annotation.id) {
            return true;
          }
          return false;
        },
      ...
      return relatedLinkingAnnotations.some((linkAnno) => {
        if (linkAnno.body && Array.isArray(linkAnno.body)) {
    ...
    return {
      canBeLinked: true, // All annotations can potentially be linked
      hasGeotag: !!geotagAnno,
      hasLinks: linkedIds.length > 0,
      hasPointSelector: hasPointSelector,
    };
  };

  const getAnnotationTitle = (annotation: any) => {
  ...
  const getAnnotationPreview = (annotation: any) => {
    const bodies = getBodies(annotation);
    if (bodies.length > 1) {
      return bodies
        .slice(1)
  ...
  return (
    <div className="h-full border-l border-border bg-card flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-2 border-b border-border text-xs text-muted-foreground bg-muted/30 flex-shrink-0">
                ...
                >
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="flex flex-col items-center gap-1 flex-shrink-0">
                      {/* Expand button with better labeling */}
                      <Button
                        variant="ghost"
                        size="sm"
