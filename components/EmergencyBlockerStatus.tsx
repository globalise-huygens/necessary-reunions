...
import { isRequestBlocked, clearAllBlocks } from '@/lib/request-blocker';

interface EmergencyBlockerStatusProps {
  className?: string;
}

export function EmergencyBlockerStatus({ className = '' }: EmergencyBlockerStatusProps) {
  ...
  return (
    <div className={`fixed bottom-4 right-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg z-50 ${className}`}>
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-bold">🚫 Request Blocker Active</h4>
          <p className="text-sm">Some API requests have been blocked due to repeated failures.</p>
