            ...
            <Card className="shadow-none">
              <CardContent className="p-3 space-y-3 text-sm">
                ...
                {manifest.homepage?.length > 0 &&
                  renderField(
                    'Homepage',
                    manifest.homepage.map((h: any) => (
                      <a
                        key={`homepage-${h.id || h['@id']}`}
                        href={h.id || h['@id']}
                        target="_blank"
                        rel="noopener noreferrer"
                ...
                {manifest.metadata?.length > 0 && (
                  <div className="border-t pt-2 mt-2 space-y-3">
                    ...
                    {manifest.metadata.map((m: any) => {
                      const v = getLocalizedValue(m.value) || '';
                      const text = v.replace(/<[^>]*>/g, '');
                      const urlMatch = text.match(/https?:\/\/\S+/);
                      const label = getLocalizedValue(m.label);
                      return (
                        <div key={`metadata-${label}-${text.slice(0, 30)}`}>
                      ...
                      if (allLabels && allLabels.length > 1) {
                        return (
                          <div>
                            ...
                            <div className="space-y-2">
                              {allLabels.map(({ language, value }) => (
                                <div key={`canvas-label-${language}-${value}`}>
                                  <div className="text-xs text-muted-foreground font-medium uppercase tracking-wide">
                                    {language}:
                                  </div>
                                  <div className="break-words whitespace-normal">
                  ...
                  {canvas.metadata?.length > 0 && (
                    <div className="border-t pt-2 mt-2 space-y-3">
                      ...
                      {canvas.metadata
                        ...
                        .map((m: any) => {
                          const label = getLocalizedValue(m.label);
                          const value = getLocalizedValue(m.value).replace(
                            /<[^>]*>/g,
                            '',
                          );
                          return (
                            <div key={`canvas-metadata-${label}`}>
                ...
                renderField(
                  ...
                  <div className="mt-1 space-y-1 text-xs font-mono">
                    {allmapsAnno.body.features.map((f: any, idx: number) => {
                      return (
                        <div
                          key={`gcp-${f.properties.resourceCoords[0]}-${f.properties.resourceCoords[1]}`}
                        >{`
