...
export function TagSelector({ onAddTag }: TagSelectorProps) {
  ...
  const handleSearchWikidata = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/wikidata?q=${encodeURIComponent(searchTerm)}`)
  ...
  const handleSearchGeoNames = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/geonames?q=${encodeURIComponent(searchTerm)}`)
      if (!response.ok) throw new Error("Search failed")
  ...
  const mockWikidataResults = [
    ...
    { id: "Q5", label: "Human", description: "Species of primate (Homo sapiens)" },
  ]
